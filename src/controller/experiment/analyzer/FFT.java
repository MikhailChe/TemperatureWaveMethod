...
public class FFT {
	...
	double avgAmplitude = indFourier.parallelStream().mapToDouble(fftdata -> getAbs(fftdata, 0)).average()
		.getAsDouble();

	double[] out = indFourier.parallelStream().map(fftdata -> {
	    double abs = getAbs(fftdata, 0);
	    return new double[] { fftdata[0] / abs, fftdata[1] / abs };
	}).reduce(new double[] { 0, 0 }, (a, b) -> {
	    a[0] += b[0];
	    a[1] += b[1];
	    return a;
	});
