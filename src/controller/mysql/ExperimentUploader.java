...
public class ExperimentUploader {
	...
	private Optional<Integer> getSampleId(Sample s) {
		Integer id = null;
		try (ResultSet result = mysql.query(
				"SELECT `id` FROM `" + sampleTableName + "`  WHERE `name` = ? AND `length` = ?", s.getName(),
				s.getLength())) {
			if (result.next()) {
				try {
					id = result.getInt("id");
	...
	private Optional<Integer> createSample(Sample s) {
		Integer id = null;
		try (ResultSet result = mysql.queryUpdate(
				"INSERT INTO `" + sampleTableName + "` SET `name`=?, `comment`=?, `length`=?", s.getName(),
	...
	boolean sampleExists(Sample s) {
		try (ResultSet rs = mysql.query(
				"SELECT * FROM `" + sampleTableName + "` WHERE `name` = ? AND `comment`=? AND `length`=?", s.getName(),
				s.getComment(), s.getLength());) {
			if (rs == null)
				return false;
			if (rs.next() == false)
	...
	private void createDataStructure() {
		mysql.queryUpdate(
				String.format("CREATE TABLE IF NOT EXISTS `%s` (" + "`id` INT NOT NULL AUTO_INCREMENT PRIMARY KEY,"
	...
	private boolean dataStructureExists() {
		try (ResultSet rs = mysql.query(String.format("SELECT 1 FROM `%s`", sampleTableName));) {
			if (rs == null)
				return false;
			if (!rs.next())
				return false;
		...
		try (ResultSet rs = mysql.query(String.format("SELECT 1 FROM `%s`", measuresTableName))) {
			if (rs == null)
				return false;
			if (!rs.next())
				return false;
	...
	private void uploadMeasurement(Measurement m, Integer id) {
		IntStream.range(0, m.diffusivity.size()).forEach(channel -> mysql.queryUpdate(
				String.format(
						"INSERT INTO `%s` SET " + "`id_sample` = ?," + "`id_channel` = ?," + "`temperature` = ?,"
								+ "`timestamp` = ?," + "`frequency` = ?," + "`amplitude` = ?," + "`diffusivity` = ?",
						measuresTableName),
				id, channel, m.temperature.get(0).value, m.time, m.frequency, m.diffusivity.get(channel).amplitude,
				m.diffusivity.get(0).diffusivity));
	}
}
