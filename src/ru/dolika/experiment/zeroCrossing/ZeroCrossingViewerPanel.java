...
public class ZeroCrossingViewerPanel extends JPanel {
	...
	final private ZeroCrossing shifts;
	...
	private void paintGrid(Graphics2D g) {
		...
		for (int i = 0; i <= 30; i++) {
			int xScreen = (int) map(i, 0, 30, 0 + MARGIN, componentWidth
					- MARGIN);
			if (i % 10 == 0) {
		...
		for (int i = -180; i <= 180; i++) {
			int yScreen = (int) map(i, -180, 180, componentHeight - MARGIN,
					MARGIN);
			if (i % 45 == 0) {
				g.setStroke(new BasicStroke(1));
				if (i % 90 == 0) {
	...
	private void paintComponent(Graphics2D g) {
		...
		for (double freq = 0.1; freq < 30; freq += 0.1) {
			int xScreen = (int) map(freq, 0, 30, 0 + MARGIN, componentWidth
					- MARGIN);
			int yScreen = (int) map(shifts.getCurrentShift(freq), -180, 180,
					componentHeight - MARGIN, 0 + MARGIN);
